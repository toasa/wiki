...
* システムタイマ：一定の間隔で周期的に動作するタイマ。指定された間隔が経過すると、カーネルは時計を１単位分進める。
* tick, jiffy：システムタイマの単位
* HZ：Linux が１秒間にシステムタイマを実行する回数。つまりタイマの周波数。HZ は 100, 250, 1000 のどれかでカーネルコンフィグで設定する。HZ が 100 の場合、1jiffy は 0.01 秒。HZ の値は `sysconf(_SC_CLK_TCK)` で調べることができる。
