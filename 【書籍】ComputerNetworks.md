...
HTTP/1.0では、各HTTPリクエストのたびに TCP コネクションを張っていた。
HTTP/1.1 では、１つの TCP コネクション確立しリクエストを送信し、レスポンスをもらった後、追加でリクエスト＆レスポンスを送信することができるようになった。これを**永続コネクション**という。

## 7.5 コンテンツ配信

### 7.5.2 サーバー・ファームとWebプロキシ

Webプロキシにより、組織（e.g. 企業、ISPなど）内のユーザ間でキャッシュを共有できる。
Web プロキシは、ユーザと組織、両方にメリットがある。
ユーザは、リクエストの応答時間が早くなる（リクエストは内部のプロキシによって処理され、外のインターネットに出ていかないため）ため嬉しい。
組織は、必要な帯域幅を減らすことができるため嬉しい。